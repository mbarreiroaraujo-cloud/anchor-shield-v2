/**
 * Security report data for the dashboard demo.
 * In production, this would be loaded from SECURITY_REPORT.json.
 * This static import ensures the dashboard works standalone.
 */
  ...
  semantic_analysis: {
    ...
    findings: [
      ...
      },
      {
        ...
        description:
          "The withdraw function only checks `user.deposited >= amount` without verifying that remaining deposits still cover outstanding borrows. There is no cross-instruction validation between withdraw and borrow. A user can deposit collateral, borrow against it, then withdraw all collateral \u2014 leaving the protocol with bad debt.",
        attack_scenario:
        ...
        estimated_impact:
          "Theft of pool funds. Attacker profits the borrowed amount minus zero risk. Protocol left with bad debt.",
        confidence: 0.98,
        source: "validated",
      },
      {
        ...
        estimated_impact:
          "Prevents liquidation of underwater positions. Protocol accumulates unrecoverable bad debt as unhealthy positions cannot be closed.",
        confidence: 0.92,
        source: "validated",
      },
      {
        ...
        description:
          'When `user.borrowed == 0` and `interest == 0`, the expression `user.deposited * 100 / (user.borrowed + interest)` divides by zero. In Rust, integer division by zero causes a panic, which in Solana translates to a program error.',
        attack_scenario:
        ...
        estimated_impact:
          "Denial of service on the liquidation function. While not directly profitable, it can block composed transactions that include liquidation checks.",
        confidence: 0.95,
        source: "validated",
      },
    ],
